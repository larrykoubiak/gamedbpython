...
class GameDBRegex:
    ...
    def get_re_result(self, regname, value):
        return self.regexes[regname].search(value)

    def get_re_results(self, regname, value):
        return self.regexes[regname].finditer(value)

    def get_cleaned_date(self, datestring):
            resultDate = None
            self.dateFormats = {}
            self.dateFormats['datRegex'] = {
                'regex': re.compile(
                    "(?P<Year>\\d{4})(?P<Month>\\d{2})(?P<Day>\\d{2})-" +
                    "(?P<Hour>\\d{2})(?P<Minute>\\d{2})(?P<Second>\\d{2})"),
                'format': '%d/%m/%Y %H:%M:%S'}
            self.dateFormats['mobyRegex'] = {
                'regex': re.compile(
                    "^(?P<Month>.+?) " +
                    "(?P<Day>\\d+?), " +
                    "(?P<Year>\\d+?)$"),
                'format': '%d/%B/%Y %H:%M:%S'}
            self.dateFormats['mobyMonthYearRegex'] = {
                'regex': re.compile("^(?P<Month>.+?), \\s(?P<Year>\\d+?)$"),
                'format': '%d/%B/%Y %H:%M:%S'}
            self.dateFormats['monthYearRegex'] = {
                'regex': re.compile("^(?P<Month>\\D+?)\\s(?P<Year>\\d+?)$"),
                'format': '%d/%B/%Y %H:%M:%S'}
            self.dateFormats['yearRegex'] = {
                'regex': re.compile("^(?P<Year>\\d+?)$"),
                'format': '%d/%m/%Y %H:%M:%S'}
            self.dateFormats['dayMonthYearRegex'] = {
                'regex': re.compile(
                    "^(?P<Month>\\d+?)\\/" +
                    "(?P<Day>\\d+?)\\/" +
                    "(?P<Year>\\d{2}?)$"),
                'format': '%d/%m/%y %H:%M:%S'}
            for dateformat in self.dateFormats.values():
