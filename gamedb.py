...
class GameDB(QObject):
    ...
    def import_dats(self):
        ...
        dat = DAT()
        dat.read_dat('libretro-database/metadat/no-intro/' +
    ...
    def import_dat(self, dat):
        ...
        sysresult = self.regexes.get_re_result("System", dat.header["name"])
        systemId = self.database.getSystem(
            sysresult.group('Manufacturer'),
            sysresult.group('Name'))
        datType = 'Standard' if sysresult.group('DatType') is None \
            else sysresult.group('DatType')
        datVersion = dat.header['version'] \
            if dat.header['version'] is not None \
            else dat.header['date']
        # check if version is a date
        datDate = self.regexes.get_cleaned_date(datVersion)
        if datDate is not None:
            datVersion = datDate
        datFileId = self.database.getDATFile(
            systemId,
            dat.filename,
            datType,
            dat.releaseGroup,
            datVersion)
        for gamekey, gamevalue in dat.softwares.items():
            datGameId = self.database.getDATGame(
                datFileId,
                gamevalue['Name'],
                gamevalue['CloneOf'],
                gamevalue['RomOf'])
            for rom in gamevalue['Roms']:
                self.database.getDATROM(
                    datFileId,
                    datGameId,
                    rom['name'],
                    rom['merge'],
    ...
    def import_new_ROMS(self):
        ...
        for datRom in datRoms:
            ...
            if systemId != datRom[0]:
                systemId = datRom[0]
                print("exporting new roms for " +
                      self.database.getSystemName(systemId))
            # get software
            ...
            self.database.getROM(releaseId, *datRom[5:])
        self.database.save()

    def import_software(self, gameName, systemId):
        softresult = self.regexes.get_re_result("Software", gameName)
        softwarename = softresult.group('Name')
        softwaretype = 'BIOS' if softresult.group('BIOS') is not None \
            else softresult.group('Type') if softresult.group('Type') \
            is not None else 'Game'
        return self.database.getSoftware(systemId, softwarename, softwaretype)

    def import_release(self, releaseName, softwareId):
        compresult = self.regexes.get_re_result("Compilation", releaseName)
        devstatusresult = self.regexes.get_re_result("DevStatus", releaseName)
        demoresult = self.regexes.get_re_result("Demo", releaseName)
        licenseresult = self.regexes.get_re_result("License", releaseName)
    ...
    def import_softwareflags(self, dat):
        for gamekey, gamevalue in dat.softwares.items():
            sysresult = self.regexes.get_re_result(
                "System",
                dat.header["name"])
            systemId = self.database.getSystem(
                sysresult.group('Manufacturer'),
            ...
            for rom in gamevalue['Roms']:
                ...
                if 'Serial' in gamevalue:
                    self.database.addReleaseFlagValue(
                        releaseId,
                        self.database.getReleaseFlag('ProductID'),
                        gamevalue['Serial'])
        self.database.save()

    def import_releaseflags(self, releaseName, releaseId):
        for regionresult in self.regexes.get_re_results("Region", releaseName):
            self.database.addReleaseFlagValue(
                releaseId,
                self.database.getReleaseFlag('Region'),
                regionresult.group('Region'))
        for languageresult in self.regexes.get_re_results(
             "Language",
             releaseName):
            self.database.addReleaseFlagValue(
                releaseId,
                self.database.getReleaseFlag('Language'),
                languageresult.group('Language'))
        versionresult = self.regexes.get_re_result("Version", releaseName)
        if(versionresult):
            self.database.addReleaseFlagValue(
                releaseId,
                self.database.getReleaseFlag('Version'),
                versionresult.group('Version'))
        revisionresult = self.regexes.get_re_result("Revision", releaseName)
        if(revisionresult):
            self.database.addReleaseFlagValue(
                releaseId,
                self.database.getReleaseFlag('Revision'),
                revisionresult.group('Revision'))
        baddumpresult = self.regexes.get_re_result("DumpStatus", releaseName)
        if(baddumpresult):
            self.database.addReleaseFlagValue(
                releaseId,
                self.database.getReleaseFlag('BadDump'),
                baddumpresult.group('BadDump'))
    ...
    def import_scrapers(self):
        ...
        for scraperline in scrapersfile:
            ...
            for scraperSystemKey, scraperSystem in list(
                 scraper.systems.items()):
                print("exporting game data for " +
                      scraper.name + " - " + scraperSystemKey)
                scraperSystemId = self.database.getScraperSystem(
                    scraperId,
                    scraperSystem['systemName'],
                    scraperSystem['systemAcronym'],
                    scraperSystem['systemURL'])
                for game in scraperSystem['systemGames'].values():
                    scraperGameId = self.database.getScraperGame(
                        scraperSystemId,
                        game['gameName'],
                        game['gameUrl'])
                    if game['gameParsed'] == 'Yes':
                        for flag in game['softwareFlags']:
                            self.database.addScraperGameFlagValue(
                                scraperGameId,
                                flag['name'],
                                flag['value'])
                        for release in game['releases']:
                            scraperReleaseId = self.database.getScraperRelease(
                                scraperGameId,
                                release['name'],
                                release['region'],
                                release['type'])
                            for flag in release['releaseFlags']:
                                self.database.addScraperReleaseFlagValue(
                                    scraperReleaseId,
                                    flag['name'],
                                    flag['value'])
                            for image in release['releaseImages']:
                                self.database.getScraperReleaseImage(
                                    scraperReleaseId,
                                    image['name'],
                                    image['type'])
        self.database.save()
    ...
    def match_softwares(self):
        ...
        for system in systems:
            ...
            releasegamelist = self.database.getScraperRelease2GameList(
                system[2])
            ...
            for software in softwares:
                scraperReleaseId = self.matcher.match_fuzzy(
                    releaseDic,
                    software[1],
                    "Full", 80)
                if scraperReleaseId is None:
                    scraperReleaseId = self.matcher.match_fuzzy(
                        releaseDic,
                        software[1],
                        "Partial",
                        86)
                if scraperReleaseId is not None:
                    self.database.addSoftwareMatch(
                        software[0],
                        gameDic[scraperReleaseId])
        self.database.save()
    ...
    def match_releases(self):
        ...
        for system in systems:
            ...
            for releaserow in releaserows:
                matches = self.database.getScraperGame2ReleaseList(
                    releaserow[2],
                    releaserow[3])
                if len(matches) == 1:
                    ...
                    releaseName = self.regexes.get_re_result(
                        'Software',
                        releaserow[1]).group("Name")
                    scraperReleaseId = self.matcher.match_fuzzy(
                        releaseDic,
                        releaseName,
                        "Full",
                        80)
                    self.database.addReleaseMatch(
                        releaserow[0],
                        scraperReleaseId)
        self.database.save()
    ...
    def match_software_flags(self):
        ...
        for flagrow in softwareflags:
            ...
            if flagvalue != "" and flagvalue is not None:
                self.database.addSoftwareFlagValue(
                    flagrow[0],
                    flagid,
                    flagvalue)
        self.database.save()
    ...
    def export_gamedbflags(self):
        ...
        for flagtuple in lstFlags:
            ...
            for systemId, systemName in systemrows.items():
                print("Exporting {0} flag {1} for system {2}".format(
                    flagtuple[2],
                    flagtuple[1],
                    systemName))
                system = {}
                ...
                system['roms'] = []
                rows = self.database.getSystemFlagValues(
                    systemId,
                    flag['srcName'])
                if systemName in [
                ...
                for row in rows:
                    ...
                    if flag['destName'] == "releasemonth":
                        rom['flagvalue'] = str(datetime.strptime(
                            row[3],
                            '%Y-%m-%d %H:%M:%S').month)
    ...
    def apply_patches(self, stage):
        patchname = "sql/patch_" + stage + ".sql"
