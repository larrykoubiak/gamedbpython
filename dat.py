...
class DAT:
            ...
            if(line == "game (\n"):
                ...
                while(line != ")\n"):
                    ...
                    if result.group(1) == "rom":
                        ...
                        for key, val in list(romresult.groupdict().items()):
                            rom[key] = val
    ...
    def import_metadat_dat(self, datfile, debug=False):
        ...
        while tokidx < len(tokens):
            ...
            if token == "clrmamepro":  # header
                ...
                tokidx += 1
        # self.softwares = OrderedDict(sorted(
        #   self.softwares.iteritems(), key=lambda s: s[0].lower()))
        self.softwares = OrderedDict(sorted(self.softwares.items()))
    ...
    def import_xml_dat(self, datfile):
        ...
        for gamenode in root.findall('game'):
            ...
            game['CloneOf'] = '' if gamenode.get('cloneof') is None \
                else gamenode.get('cloneof')
            game['RomOf'] = '' if gamenode.get('romof') is None \
                else gamenode.get('romof')
            game['Description'] = gamenode.find('description').text
            game['Roms'] = []
            for romnode in gamenode.findall('rom'):
                rom = {}
                for key, val in list(romnode.attrib.items()):
    ...
    def write_sorted_dat(self, path):
        ...
        for key, value in self.header.items():
            output.write("\t{0} {1}\n".format(key, value))
        output.write(")\n")
        for softkey, softvalue in self.softwares.items():
            output.write("\ngame (")
            ...
            for rom in softvalue['Roms']:
                output.write("\n\trom (")
                for romkey, romvalue in rom.items():
                    if romkey in (
                     'name',
                     'size',
                     'crc',
                     ...
                     'sha1',
                     'status',
                     'flags') and romvalue is not None:
                        output.write(" {0} {1}".format(romkey, romvalue))
                    elif romvalue is not None:
                        if len(rom) == 1:
                            output.write(" {0} {1}".format(romkey, romvalue))
                        else:
                            output.write("\n\t\t{0} {1}".format(
