    ...
    def GenerateScript(self, path, stage):
        ...
        for sql in [s for s in self.sqls if s['Stage'] == stage]:
            ...
            if sql["Action"] == "DELETE":
                ...
                strformat = "UPDATE {totable} SET {tofield1} = " + \
                            "(SELECT {fromfield1} FROM {fromtable} " + \
                            "WHERE {fromkey} = {fromvalue}) " + \
                            "WHERE {tokey} = {tovalue}"
                script = strformat.format(**sql)
            elif sql["Action"] == "DELETELIST":
                strformat = "DELETE FROM {totable} WHERE {tokey} IN " + \
                            "(SELECT {fromfield1} FROM {fromtable} " + \
                            "WHERE {fromkey} = {fromvalue})"
                script = strformat.format(**sql)
...
if __name__ == "__main__":
    patcher = Patcher("patches.xlsx")
    patcher.GenerateScript("sql/patch_softwaremap.sql", "SoftwareMap")
    patcher.GenerateScript("sql/patch_softwareflagmap.sql", "SoftwareFlagMap")
